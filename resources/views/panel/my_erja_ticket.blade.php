@php
$data=(new \App\Http\Controllers\ApiController())->get_my_erja_ticket()['result'];

@endphp

<h6 class="my-2 border-bottom pb-2">
    تیکت های ارجاعی به من
</h6>
...
@foreach($data as $item)
    <a href="{{route('page',['show_ticket','code'=>$item['code']])}}" class="item my-2 border p-3 rounded-2 text-center position-relative" style="display: block">
        ...
        <div class="d-flex justify-content-center align-items-center">
            <h6 class="ms-1 me-1" style="font-size: 0.7rem;color: #575757">ارجاع شده توسط :
                {{$item['user_fa']}}
            </h6>
            ---
